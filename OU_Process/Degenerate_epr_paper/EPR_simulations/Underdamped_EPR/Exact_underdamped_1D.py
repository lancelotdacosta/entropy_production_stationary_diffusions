'''
We perform an exact simulation of the 1D underdamped Langevin dynamics in a quadratic potential
We check numerically that the transition kernels as time goes forward or backward are equivalent
We check that the ep as a function of the time-step is always infinity
...
plt.savefig("sample_trajectory_1D_underdamped.png")
...
plt.scatter(x[0, :], x[1, :], marker='o', c='darkorange', alpha= 0.3)
orange_patch = Line2D([0], [0], marker='o', color='w', label=r'Forward samples $\sim p_\epsilon(\cdot, x_0)$',markerfacecolor='darkorange', markersize=8)

plt.scatter(x_rev[0, :], x_rev[1, :], marker='o', c='cornflowerblue',
           label=r'Backward samples $\sim \bar{p}_\epsilon(\cdot, x_0)$',alpha= 0.3)
blue_patch = Line2D([0], [0], marker='o', color='w', label=r'Backward samples $\sim \bar{p}_\epsilon(\cdot, x_0)$',markerfacecolor='cornflowerblue', markersize=8)
...
plt.legend(handles=[orange_patch,blue_patch], loc="upper center")
plt.scatter(x0[0], x0[1], marker='o', c='red', s =20)
plt.text(x0[0], x0[1],s='$\mathbf{x_0}$')
plt.savefig("transition_kernels_underdamped_1D.png")
...
plt.savefig("ep_timestep_1D_underdamped.png")
