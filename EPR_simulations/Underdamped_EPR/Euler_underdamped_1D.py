'''
We perform an Euler simulation of the 1D underdamped Langevin dynamics in a quadratic potential
We check numerically that the transition kernels as time goes forward or backward are singular
'''
...
#In other words, dimension 0 of the process is positions and dimension 1 is momenta
...
plt.xlabel('q')
plt.ylabel('p')
plt.title('Sample trajectory \n E-M discretisation of underdamped Langevin')
#ax.legend() #loc="upper right"
plt.savefig("sample_trajectory_1D_underdamped_Euler.png")
...
N = int(2*10 ** 2)  # number of trajectories
epsilon = 0.01
...
orange_patch = Line2D([0], [0], marker='o', color='w', label=r'Forward samples $\sim p^{E-M}_\epsilon(\cdot, x_0)$',markerfacecolor='darkorange', markersize=8)

# support of backward transition kernel: q and p coordinates #comput 6.9.2022
q_supp_neg_p = np.linspace(inf_q,sup_q,2)
p_supp_neg_p = (-q_supp_neg_p+x0[0])/epsilon
plt.plot(q_supp_neg_p, p_supp_neg_p, c='cornflowerblue')#, alpha= 1, label=r'supp $\bar{p}^{E-M}_\epsilon(\cdot, x_0)$')
blue_patch = Line2D([0], [0],marker='_', label=r'supp $\bar{p}^{E-M}_\epsilon(\cdot, x_0)$',markerfacecolor='cornflowerblue', markersize=15)
...
plt.legend(handles=[black_patch, orange_patch,blue_patch], loc="upper center")
plt.scatter(x0[0], x0[1], marker='o', c='red', s =20)
plt.text(x0[0], x0[1],s='$\mathbf{x_0}$')
plt.savefig("transition_kernels_underdamped_1D_Euler.png")
